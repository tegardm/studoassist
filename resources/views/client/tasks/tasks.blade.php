...
<div class="container-fluid px-1 py-5 mx-auto">
    <div class="row d-flex justify-content-center">
        ...
        @endif
            <div class="card">
                <form class="form-card" id="taskForm" action="/submit-form" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="row justify-content-between text-left">
                        <div class="form-group flex-column d-flex">
                            <label class="form-control-label px-3">Masukkan Judul Tugas<span class="text-danger"> *</span></label>
                            <input type="text" id="title" name="title" placeholder="Masukan Nama Tugas" onblur="validate(1)">
                        </div>
                    </div>
                    <div class="row justify-content-between text-left">
                        <div class="form-group col-sm-6 flex-column d-flex">
                            <label class="form-control-label px-3">Deadline<span class="text-danger"> *</span></label>
                            <input type="date" id="deadline" name="deadline" placeholder="" onblur="validate(3)">
                        </div>
                        <div class="form-group col-sm-6 flex-column d-flex">
                            <label class="form-control-label px-3">Modul PDF (Optional)<span class="text-danger"> *</span></label>
                            <input type="file" id="pdfModule" name="pdfModule" accept=".pdf" onchange="validate(4)">
                        </div>
                    </div>
                    <div class="row justify-content-between text-left">
                        <div class="form-group col-sm-6 flex-column d-flex">
                            <label class="form-control-label px-3">Mata Kuliah<span class="text-danger"> *</span></label>
                            <select required id="course" class="p-2" name="course_id" onblur="validate(5)">
                             
                                @foreach ($data as $mk)
                                <option value="{{$mk->course_code}}">{{$mk->title}}</option>
                                @endforeach
                    ...
                    <div class="row justify-content-between text-left">
                        <div class="form-group col-12 flex-column d-flex">
                            ...
                            <textarea id="description" name="description" placeholder="" onblur="validate(6)"></textarea>
                        </div>
                    </div>
